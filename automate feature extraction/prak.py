...
audio_segment = AudioSegment.from_wav('Prakriti//audio.wav')
normalized_sound = match_target_amplitude(audio_segment, -20.0)

print("length of audio_segment={} seconds".format(len(normalized_sound)/1000))

nonsilent_data = detect_nonsilent(normalized_sound, min_silence_len=4500, silence_thresh=-25, seek_step=1)
...
for i, chunks in enumerate(nonsilent_data):
	...
	audio_chunk1.export( "Prakriti"+"//Audio Record//"+"answer_chunk{}.wav".format(i), format="wav")
	# if flg==1:
		# prev = chunks[0]
	df.loc[len(df)] = [chunks[0], chunks[1]]
	list1.append((chunks[0], chunks[1]))
	# nex = chunks[1]
	# flg=1
df.loc[len(df)] = [nex,len(normalized_sound)]
# list1.append((nex,len(normalized_sound)))
df.to_csv("Prakriti//timestamps_4500.csv",header= False)
# print(list1)
